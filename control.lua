...
function rr.getItemProtoFromName(n) return prototypes.item[n] end

function rr.setGUISize(element, w, h)
    if not element.style then return end
    if w then element.style.width = w end
    if h then element.style.height = h end
end

function rr.getAllIdleRobotsInNetwork(logistic_network)
    local robots = {}
        ...
        for i = 1, #inv do
            ...
            if itemstack.valid_for_read then
                if robots[itemstack.name] then
                    robots[itemstack.name].count = robots[itemstack.name].count + itemstack.count
                else
...
function rr.getDistanceBetweenVectors(a, b)
    local x = a.x - b.x
    local y = a.y - b.y
...
function rr.getBotsBeingRecalled(recall_chest)
    local tbl = {}
    for k, v in pairs(storage.teleportQueue) do
        if tbl.destination == recall_chest then
            tbl[v.itemname] = tbl[v.itemname] and tbl[v.itemname] + v.count or v.count
        end
    end
    return tbl
end

function rr.addToTeleportQueue(source, destination, itemstack)
    local currentTick = game.tick
    local destinationInv = destination.get_inventory(defines.inventory.chest)
    local dist = rr.getDistanceBetweenVectors(source.position, destination.position)

    local robotEnt = itemstack.prototype.place_result
    local unitsPerTick = (source["force"]["worker_robots_speed_modifier"] + 1)
        * robotEnt.speed
        * settings.global["recall-speed-modifier"].value
...
function rr.buildRecallGui(baseGUI, entity)
    if not entity then return end
    if entity.name ~= "robot-recall-chest" then return end
    local logistic_network = entity.logistic_network
    if baseGUI["robot-recall-chest"] ~= nil then baseGUI["robot-recall-chest"].destroy() end
    ...
    rr.setGUISize(recallFrame, WIDTH, HEIGHT)
    rr.setGUISize(recallScrollFlow, WIDTH - 20, HEIGHT - 50)
    -- game.print(ply.gui)
    local ply = game.players[baseGUI.player_index]
    local res = ply.display_resolution
    local scl = ply.display_scale
    storage.openedGUIPlayers[baseGUI.player_index] = { ply = game.players[baseGUI.player_index], ent = entity }
    recallFrame.location = {
        (res.width / 2) - (INV_DIMENSIONS.width * scl * 0.5) - WIDTH * scl,
        (res.height / 2) - (INV_DIMENSIONS.height / 2 * scl) + (INV_DIMENSIONS.verticalOffset * scl),
    }

    local robots = rr.getAllIdleRobotsInNetwork(logistic_network)
    rr.updateRecallGuiList(baseGUI, robots, logistic_network)
end

function rr.updateRecallGuiListEntry(itemname, count, baseGui)
    local scrollPane = baseGui["robot-recall-chest"]["frame"]["scrollpane"]
    local ply = game.players[baseGui.player_index]
    local flow = baseGui["robot-recall-chest"]["frame"]["scrollpane"][itemname]
        or scrollPane.add({ type = "flow", name = itemname })
    local spritebutton = flow["spritebutton"]
        or flow.add({
            type = "sprite-button",
            tooltip = {
                "robot-recall.recall-button-tooltip",
                rr.getItemProtoFromName(itemname).localised_name,
            },
            name = "spritebutton",
        })

    if spritebutton.sprite == "" then spritebutton.sprite = "item/" .. itemname end

    local progressbar = baseGui["robot-recall-chest"]["frame"]["scrollpane"][itemname .. "-progressbar"]
        or scrollPane.add({
            type = "progressbar",
    ...
    if ply.opened and ply.opened.name == "robot-recall-chest" then
        if ply.opened.get_inventory(defines.inventory.chest).can_insert({ name = itemname }) then
    ...
    label.caption = { "robot-recall.recall-count", rr.getItemProtoFromName(itemname).localised_name, count }
    label.style.single_line = false
end

function rr.updateRecallGuiList(baseGui, robots, logistic_network)
    local scrollPane = baseGui["robot-recall-chest"]["frame"]["scrollpane"]
    local ply = game.players[baseGui.player_index]
    if logistic_network == nil or not logistic_network.valid then
        local label = scrollPane["no-network"]
            or scrollPane.add({
                type = "label",
    ...
    if scrollPane["no-network"] and scrollPane["no-network"].valid then scrollPane["no-network"].destroy() end
    local recalled = rr.getBotsBeingRecalled()
    for k, v in pairs(recalled) do
        if robots[k] then
            ...
            robots[k] = {
                count = v,
                item = rr.getItemProtoFromName(k),
                ent = rr.getItemProtoFromName(k).place_result,
            }
        end
    end
    for k, v in pairs(robots) do
        rr.updateRecallGuiListEntry(k, v.count, baseGui)
    end

    local count = table_size(robots)
    for k, v in pairs(baseGui["robot-recall-chest"]["frame"]["scrollpane"].children) do
        if v.valid and v.type == "flow" and not robots[v.name] then
            local progress = baseGui["robot-recall-chest"]["frame"]["scrollpane"][v.name .. "-progressbar"]
            v.destroy()
            progress.destroy()
    ...
    if count == 0 and not scrollPane["no-robots-label"] then
        ...
        label.style.width = scrollPane.style.maximal_width - 10
        return
    elseif count > 0 and scrollPane["no-robots-label"] and scrollPane["no-robots-label"].valid then
...
function rr.updateRecallGuiListProgress(baseGui, robots, logistic_network)
    ...
    for _, element in pairs(scrollPane.children) do
        if element.type == "progressbar" then
            ...
            local itemname = string.sub(element.name, 0, -1 - string.len("-progressbar"))
            local totalProgress = {}
            for k, v in pairs(storage.teleportQueue) do
            ...
            if table_size(totalProgress) ~= 0 then
                ...
                local newprog = rr.getAverage(totalProgress)
...
function rr.createRobotRecallGUI(ent, ply, gui)
    -- if (not (ent and ent.name == "robot-recall-chest")) then return end
    if gui["robot-recall-chest"] ~= nil then gui["robot-recall-chest"].destroy() end
...
function rr.callRobotsToEntity(location_ent, logisticNetwork, robotItemName, quality)
    for k, cell in pairs(logisticNetwork.cells) do
        ...
        for i = 1, #inv do
            local itemstack = inv[i]
            if itemstack.valid_for_read then
                if itemstack.prototype.name == robotItemName and (not quality or itemstack.quality == quality) then
                    rr.addToTeleportQueue(roboport, location_ent, itemstack)
                end
            end
        end
    end
...
function rr.updateTeleportJobs(event)
    local warning = false
    for k, e in ipairs(storage.teleportQueue) do
        ...
        if (not e.destination or not e.destination.valid) or (not e.source or not e.source.valid) then
            ...
            if e.source.valid then
                count = e.source.insert({ name = e.itemname, count = e.count, quality = e.quality })
            end
            if not count or count ~= e.count then
                count = count and (e.count - count) or e.count
                e.surface.spill_item_stack(e.srcPos, { name = e.itemname, count = count, quality = e.quality })
                game.print(
                    count .. " robots have been dropped at their source position, there is no room in the source."
                )
            end
        ...
        if event.tick >= e.endTick then
            ...
            -- if () then return end
            local destinationInv = e.destination.get_inventory(defines.inventory.roboport_robot)
                or e.destination.get_inventory(defines.inventory.chest)
            local sourceInv = e.source.get_inventory(defines.inventory.roboport_robot)
                or e.source.get_inventory(defines.inventory.chest)

            -- if (destinationInv.can_insert({name = e.itemname, count = e.count})) then
            local amnt = destinationInv.insert({ name = e.itemname, count = e.count, quality = e.quality })
            if amnt ~= e.count then
                local initialRemainder = e.count - amnt
                local remainder = e.count - amnt
                remainder = remainder - sourceInv.insert({ name = e.itemname, count = remainder, quality = e.quality })
                if remainder ~= 0 then
                    game.print(
                        "Recall of "
                            .. e.count
                            .. " '"
                            .. e.itemname
                            .. "' robots has (partially) failed. "
                            .. remainder
                            .. " could not be recalled."
                    )
                    if e.destination.logistic_network and e.destination.logistic_network.valid then
                        for _, cell in pairs(e.destination.logistic_network.cells) do
                            if remainder ~= 0 then
                                local inv = cell.owner.get_inventory(defines.inventory.roboport_robot)
                                remainder = remainder - inv.insert({ name = e.itemname, count = remainder })
                            end
                        end
                        if remainder ~= initialRemainder then
                            -- game.print("Recall of " .. e.count .. " " .. e.itemname .. " has failed.")
                            game.print((initialRemainder - remainder) .. " robots have been redeployed to roboports")
                        end
                    end

                    if remainder ~= 0 then
                        game.print(remainder .. " robots have been dropped in front of the recall station.")
                        local ent = e.destination.surface.spill_item_stack(
                            e.destination.position,
...
function rr.initRecallChest(event)
...
script.on_event(defines.events.on_gui_click, function(event)
    ...
    if event.element.type == "sprite-button" and ply.opened and ply.opened.name == "robot-recall-chest" then
        local itemname = event.element.parent.name
        local item = prototypes.item[itemname]
        -- game.print('recalling "' .. itemname .. '"')
        rr.callRobotsToEntity(ply.opened, ply.opened.logistic_network, item)
    end

    if event.element.name == "robot-recall-chest.close" then event.element.parent.destroy() end
end)
...
script.on_nth_tick(2, function(event)
    ...
    if storage.teleportQueueEntryCount == 0 and storage.hasChanged then
        ...
        for k, v in pairs(storage.openedGUIPlayers) do
            rr.updateRecallGuiListProgress(v.ply.gui.screen)
        end
...
script.on_nth_tick(10, function(event)
    ...
    if storage.teleportQueueEntryCount and storage.teleportQueueEntryCount > 0 then rr.updateTeleportJobs(event) end
end)

local function tick_struct(struct)
    local signals =
        struct.entity.get_signals(defines.wire_connector_id.circuit_red, defines.wire_connector_id.circuit_green)
    for _, _signal in ipairs(signals or {}) do
        local signal = _signal.signal
        if signal.name:find("logistic%-robot") ~= nil or signal.name:find("construction%-robot") ~= nil then
            remote.call("robot-recall-space-age", "call_robots_to_entity", struct.entity, signal.name, signal.quality)
        end
    end
end
...
script.on_event(defines.events.on_object_destroyed, function(event)
    ...
    if deathrattle then
        ...
        if deathrattle[1] == "struct" then storage.structs[deathrattle[2]] = nil end
    end
end)

remote.add_interface("robot-recall-space-age", {
    call_robots_to_entity = function(entity, robotItemName, quality)
        rr.callRobotsToEntity(entity, entity.logistic_network, robotItemName, quality)
    end,
})
-- require('')
...
for _, event in ipairs({
    ...
    script.on_event(event, rr.initRecallChest, { { filter = "name", name = "robot-recall-chest" } })
end
