...
function rr.callRobotsToEntity(location_ent, logisticNetwork, robotItem)
    for k, cell in pairs(logisticNetwork.cells) do
        ...
        for i = 1, #inv do
            local itemstack = inv[i]
            if itemstack.valid_for_read then
                if itemstack.prototype == robotItem then rr.addToTeleportQueue(roboport, location_ent, itemstack) end
            end
        end
    end
end
...
function rr.initRecallChest(event) end

script.on_event(defines.events.on_built_entity, function(event)
    -- game.print("Hello!")
    if event.entity and event.entity.name == "robot-recall-chest" then rr.initRecallChest(event) end
end)
