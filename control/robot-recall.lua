...
function rr.getItemProtoFromName(n)
	return prototypes.item[n]
end
...
function rr.buildRecallGui(baseGUI, entity)
	...
	if entity.name ~= "robot-recall-chest" then
		return
	end
...
function rr.updateRecallGuiListProgress(baseGui, robots, logistic_network)
	if not baseGui or not baseGui["robot-recall-chest"] then
		return
	end
	...
	for k, e in ipairs(storage.teleportQueue) do
		...
		if not e.destination or not e.destination.valid then
			...
			if e.source.valid then
				count = e.source.insert({ name = e.itemname, count = e.count })
			end
...
script.on_event(defines.events.on_built_entity, function(event)
	-- game.print("Hello!")
	if event.entity and event.entity.name == "robot-recall-chest" then
		rr.initRecallChest(event)
	end
end)
...
script.on_nth_tick(2, function(event)
	if not storage.teleportQueueEntryCount or not storage.teleportQueue then
		return
	end
...
script.on_nth_tick(10, function(event)
	...
	if storage.teleportQueueEntryCount and storage.teleportQueueEntryCount > 0 then
		rr.updateTeleportJobs(event)
	end
end)
