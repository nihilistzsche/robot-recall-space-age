script.on_nth_tick(15, function(event)
	for k, v in pairs(storage.deploying) do
		if v.deploying and v.ent.valid then
			local inv = v.ent.get_inventory(defines.inventory.item_main)
			if not inv.is_empty() then
				local items = inv.get_contents()
				for _, info in pairs(items) do
					local placeresult = prototypes.item[info.name].place_result
					if
						placeresult
						and (placeresult.type == "logistic-robot" or placeresult.type == "construction-robot")
					then
						if logisticNetworkHasItemSpace(v.ent.logistic_network, info.name) then
							...
							inv.remove({ name = info.name, count = 1, quality = info.quality })
						end
					end
				end
			end
...
script.on_nth_tick(60, function(event)
	for k, v in pairs(storage.deploying) do
		if v.ent and not v.ent.valid then
			table.remove(storage.deploying, k)
		elseif v.ent and not v.ent.get_inventory(1).is_empty() then
			v.deploying = true
		else
			v.deploying = false
		end
